...
export const setWindowTitle = (title: string = null) => {
	const t =
		title || window.location.pathname?.split("/")?.slice(2)?.join(" | ");
...
export const dateAction = (prev: number, type: string) => {
  let date: any;
  if (type === "day")
    date = new Date(new Date().setDate(new Date().getDate() - prev));

  if (type === "month")
    date = new Date(new Date().setMonth(new Date().getMonth() - prev));
...
export const dateCompare = (sd, ed) => {
  ...
  if (yesterday === sDate && (today === eDate || yesterday === eDate))
    return "Yesterday";
  ...
  return (
    ...
    " - " +
    generateDateFormat(
      new Date(ed ? eDate : new Date()).toLocaleDateString(),
      "%date% %MM% %yyyy%"
    )
  );
};
