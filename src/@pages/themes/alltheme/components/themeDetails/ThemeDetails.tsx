...
import { ITheme, IThemeRelease } from '@interfaces/themeCustomization.interface';
...
const ThemesDetails = () => {
	...
	useEffect(() => {
		...
		Promise.allSettled([themeDetailsReq, themeReleaseReq, themeCheckInstalledReq, themeListReq, themeFAQReq])
			.then(([themeDetailsRes, themeReleaseRes, themeCheckInstalledRes, themeListRes, themeFAQRes]) => {
				themeDetailsRes.status === 'fulfilled' && setThemeData(themeDetailsRes.value?.body);
				themeReleaseRes.status === 'fulfilled' && setReleaseInfo(themeReleaseRes.value?.body);
				themeCheckInstalledRes.status === 'fulfilled' && setInstallStatus(themeCheckInstalledRes.value?.body);
				themeListRes.status === 'fulfilled' && setThemeList(themeListRes.value?.body);
				themeFAQRes.status === 'fulfilled' && setThemeFAQ(themeFAQRes.value?.body);
			})
	...
	const [activeTab, setActiveTab] = useState<number>(routeList.indexOf(location?.pathname));
	...
	const onInstallOrUninstall = () => {
		...
		const req = !installStatus?.isInstalled ? ThemeService.themeInstall : ThemeService.themeUninstall;
		req({ body: { themeId: id } })
			.then((resp) => {
			...
			.catch((err) => {
				installStatus?.isInstalled ? ToastService.warning(err.message) : ToastService.error(err.message);
			})
				...
				{isLoading ? (
					...
					<div className='row  pb-0'>
						<div className='col-lg-5 col-md-6 col-sm-12 wx__details_top_left d-flex align-items-center'>
							<div>
								<Thumbnail src={imageURLGenerate(themeDetails?.themeIcon)} noBorder />
								...
								<div className='d-flex align-items-center gap-3'>
									<Button
										...
										disabled={isInstalling || activePlan?.level < themeDetails?.packageLevel}
									>
										{isInstalling ? <Preloader /> : installStatus?.isInstalled ? 'Uninstall' : 'Install'}
									</Button>
									{installStatus?.isInstalled &&
										(installStatus?.isPublished ? (
											...
											<Button
												...
												className='p-2'
												onClick={onPublish}
												disabled={activePlan?.level < themeDetails?.packageLevel}
											>
										...
										<div className='wx__fqa'>
											{themeFAQ?.length ? (
												<>
													...
													<WxAccordion data={themeFAQ} labelKey='faqQuestion' descriptionKey='faqAnswer' />
												</>
											) : (
												<h6 className='text-muted text_italic text-center'>No FAQ found!</h6>
											)}
										</div>
									}
								/>
							...
							<ul>
								...
								<li className='d-flex justify-content-between align-items-center'>
									...
									<p className='info_value '>
										{generateDateFormat(themeReleaseInfo?.fileUploadDate, '%MM% %date%, %yyyy%')}
									</p>
								</li>
								<li className='d-flex justify-content-between align-items-center'>
									...
									<p className='info_value '>
										{themeReleaseInfo?.lastUploadDate &&
											generateDateFormat(themeReleaseInfo?.lastUploadDate, '%MM% %date%, %yyyy%')}
									</p>
								</li>
								<li className='d-flex justify-content-between align-items-center'>
								...
								<li className='d-flex justify-content-between align-items-center'>
									...
									<p className='info_value '>{themeReleaseInfo?.totalInstalledUser || 0}</p>
								</li>
